...
class Main:
    def __init__(self):
        ...
        self.grid = [[0 for i in range(self.settings.maze_width)] for j in range(self.settings.maze_height)]
        self.maze = Maze(self)
    ...
    def create_buttons(self):
        ...
        width = 0.3*width_all
        self.button_simple_dijkstra_rect = pygame.Rect(center_x - 1.5*width - gap_w, center_y - gap_h- height, width, height)
        self.button_dijkstra_rect = pygame.Rect(center_x - 0.5*width, center_y - gap_h- height, width, height)
        self.button_astar_rect = pygame.Rect(center_x + 0.5*width + gap_w, center_y - gap_h- height, width, height)
        self.button_all_rect = pygame.Rect(center_x - width_all//2, center_y  + gap_h, width_all, height)
        font = pygame.font.SysFont(self.settings.font, 20, bold=True)
        self.button_simple_dijkstra_text = font.render("simple_dijkstra", True, self.settings.font_color) 
        ...
        self.screen.blit(self.button_simple_dijkstra_text, (self.button_simple_dijkstra_rect.centerx - 
            self.button_simple_dijkstra_text.get_width() // 2, self.button_simple_dijkstra_rect.centery - self.button_simple_dijkstra_text.get_height() // 2))
        pygame.draw.rect(self.screen, self.settings.button_color,   self.button_dijkstra_rect)
        self.screen.blit( self.button_dijkstra_text, ( self.button_dijkstra_rect.centerx -  
            self.button_dijkstra_text.get_width() // 2,  self.button_dijkstra_rect.centery -  self.button_dijkstra_text.get_height() // 2))
        pygame.draw.rect(self.screen, self.settings.button_color, self.button_astar_rect)
        self.screen.blit(self.button_astar_text, (self.button_astar_rect.centerx - self.button_astar_text.get_width() // 2,
            self.button_astar_rect.centery - self.button_astar_text.get_height() // 2)) 
        pygame.draw.rect(self.screen, self.settings.button_color, self.button_all_rect)  
        self.screen.blit(self.button_all_text, (self.button_all_rect.centerx - self.button_all_text.get_width() // 2,
        ...
        while flag:           
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.button_simple_dijkstra_rect.collidepoint(event.pos):
